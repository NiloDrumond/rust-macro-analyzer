...
export function MacroDefinitionsPerCrateLinesNormalized() {
  const { data } = useData();
  const chartData = React.useMemo(() => {
    if (!data) return [];
    return data.macro_definitions_per_crate
      .map(([path, count]) => {
  ...
  return (
    <Card className="col-span-1">
      <h2>Macro Definitions Per Crate (Top 100) - Lines Normalized</h2>
